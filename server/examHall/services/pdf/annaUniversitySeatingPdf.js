...
const ANNA_UNIV_LOGO_PATH = path.join(ASSETS_DIR, 'anna-univ-logo.png');
...
/**
 * Renders a single hall page for Anna University Examination in A4 Landscape.
 *
 * Layout:
 *   [Logo] ANNA UNIVERSITY THEORY EXAMINATION ... (one header group)
 *   Seating Arrangement (underlined)
 *   Info Box: College code and name | Hall No. / Date / Session
 *   Seating Boxes (dynamic from layoutType)
 *   Summary Table (Degree & Branch, Subject Code, merged Hall No.)
 *   Signatures: Name & Signature of Hall Superintendent / Signature of Chief Superintendent
 *   Page No / Footer
 */
function renderAnnaUniversityHallPage(doc, { session, hallData, pageIndex = 1, totalPages = 1 }) {
  ...
  // ===================== 1. HEADER (Logo + Exam Title as ONE group) =====================
  let currentY = 12;

  // Dynamic exam title from ExamMaster
  const master = session.examMaster || {};
  const examTitle = (session.examName || master.examName || 'ANNA UNIVERSITY THEORY EXAMINATION NOV./DEC. -2026').toUpperCase();

  // Measure the title text height to vertically center the logo
  doc.font('Helvetica-Bold').fontSize(13).fillColor('#000000');
  const titleTextHeight = doc.heightOfString(examTitle, { width: contentWidth - 80 });
  const headerGroupHeight = Math.max(titleTextHeight + 4, 38);

  // Anna University logo: positioned to the LEFT of the title, vertically centered in the header group
  if (fs.existsSync(ANNA_UNIV_LOGO_PATH)) {
    const logoSize = 36;
    // Measure the title width to position the logo just to the left of the centered text block
    const titleMeasuredWidth = doc.widthOfString(examTitle);
    const titleBlockLeft = (pageWidth - Math.min(titleMeasuredWidth, contentWidth - 80)) / 2;
    const logoX = titleBlockLeft - logoSize - 10;
    const logoY = currentY + (headerGroupHeight - logoSize) / 2;
    doc.image(ANNA_UNIV_LOGO_PATH, Math.max(left, logoX), logoY, { width: logoSize, height: logoSize });
  }

  // Centered exam title
  doc.font('Helvetica-Bold').fontSize(13).fillColor('#000000');
  doc.text(examTitle, left, currentY + (headerGroupHeight - titleTextHeight) / 2, {
    width: contentWidth,
    align: 'center',
  });

  currentY += headerGroupHeight + 2;
  ...
  // Reserve space below for: summary (~90pt) + gap (22pt) + signatures (22pt) + pageNo (16pt) + footer (16pt) + bottom (10pt)
  const reservedBelow = 175;
  ...
  // ===================== 4. SUMMARY TABLE =====================
  const summaryRows = AllocationService.computeDegreeBranchSummary(hallData.seats || [], 'ANNA_UNIVERSITY');

  const sumCol0 = 80;  // Hall No.
  const sumCol1 = 245; // Degree & Branch
  ...
  // Calculate dynamic data row heights
  ...
  // Vertically merged Hall No. text
  ...
  evaluatedRows.forEach((r, idx) => {
    ...
    // Horizontal row separator (only between rows, from col 1 to end)
  ...
  const sigY = Math.max(currentY + 48, pageHeight - 75);

  doc.font('Helvetica-Bold').fontSize(8.5).fillColor('#000000');
  // Left signature — centered within left block
  doc.text('Name & Signature of Hall\nSuperintendent', sigLeftX, sigY, {
    width: sigBlockWidth,
    align: 'center',
    lineGap: 1.5,
  });
  // Right signature — centered within right block
  doc.text('Signature of Chief\nSuperintendent', sigRightX, sigY, {
    width: sigBlockWidth,
    align: 'center',
    lineGap: 1.5,
  });
